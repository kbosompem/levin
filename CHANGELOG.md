...
### Changed
- Updated publisher ID to match marketplace account

## [0.6.1] - 2024-12-09

### Fixed

- **Remote Connection Stability** - Fixed "multiple LMDB connections" error
  - Added mutex locks for remote database connections
  - Serialized requests to same remote database to prevent connection storms
  - Auto-retry on connection errors with exponential backoff

- **Security** - Hide credentials in remote database display
  - Remote URIs now show only `host:port/database` instead of full credentials
  - Credentials hidden in tree view tooltips and descriptions

- **Extension Activation** - Fixed extension failing to activate
  - Removed jsedn dependency with deep transitive dependencies
  - Simplified dependency tree to just edn-data

### Changed

- **Reduced Logging Verbosity** - Cleaner output during retries
  - Full query logged only on first attempt
  - Compact single-line retry messages
  - Removed popup warnings for each retry attempt

### Documentation

- **Cross-Platform Installation** - Added detailed installation instructions
  - macOS (Homebrew and manual)
  - Linux (x64 and ARM64)
  - Windows (with PATH configuration)
